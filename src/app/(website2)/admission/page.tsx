...
function ApplicationPage() {
  ...
  const [requestId, setRequestId] = useState<string | null>(localStorage.getItem("REQUEST_ID"));
  ...
  return (
    <SectionWrapper>
      ...
      <div className="">
        ...
        {step === 2 && (
          <ParentForm step={step} requestId={requestId} nextStep={nextStep} prevStep={prevStep} />
        )}
        {step === 3 && (
          <>
            <div className="w-max ml-auto min-[340px]:min-w-[280px]">
              <p className="text-base font-semibold text-secondary max-sm:text-end">Bank Details</p>
              ...
              <div className="mt-3 mb-6 rounded-md shadow-sm bg-[#A1CDFF] p-3  flex-column gap-2">
                {bank_details?.map(({ label, value }, idx) => (
                  ...
                  >
                    ...
                    <span className="font-normal text-sm tracking-wide">{value}</span>
                  </p>
                ))}
              </div>
            </div>
